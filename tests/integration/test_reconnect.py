"""The exporter must recover on its own after the coordinator becomes
briefly unreachable (a restart during a deployment, a network blip, ...) --
see conftest.py for why `restart_coordinator` simulates both the same way,
and exporter._poll's docstring for why this isn't free: no labgrid
ClientSession reconnects on its own.
"""
...
def test_exporter_reconnects_after_coordinator_restart(
    ...
    # come back up, and get rediscovered by the exporter's own poll loop.
    ...
    # acquisition to have survived -- what actually matters is that the
    # exporter keeps correctly tracking new operations after reconnecting.
    labgrid_client("-p", PLACE, "acquire")
    try:
        wait_until(lambda: metric_value("labgrid_place_acquired", place=PLACE) == 1.0)
    finally:
