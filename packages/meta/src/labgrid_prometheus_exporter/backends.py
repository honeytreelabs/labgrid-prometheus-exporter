...
labgrid_prometheus_exporter_backend_grpc.plugin for the shape). Exactly one
...
_BACKENDS = {
    "grpc": "labgrid_prometheus_exporter_backend_grpc.plugin",
    "wamp": "labgrid_prometheus_exporter_backend_wamp.plugin",
}
...
def resolve_backend_module() -> str:
    ...
    if len(installed) > 1:
        names = ", ".join(
            f"labgrid-prometheus-exporter-backend-{extra}" for extra in sorted(installed)
        )
        raise RuntimeError(
