...
Agent-facing operational notes for `labgrid-prometheus-exporter`. For what
...
`packages/backend-grpc` (labgrid >= 25.0) and `packages/backend-wamp`
...
- `packages/core` — transport-agnostic: Prometheus metric definitions
  (`metrics.py`), the poll/reconnect loop (`exporter.py`), and the
  `CoordinatorBackend`/`Place` contract (`interface.py`). No dependency on
  `labgrid`.
- `packages/backend-grpc`, `packages/backend-wamp` — one `CoordinatorBackend`
...
- `packages/meta` — the actual `labgrid-prometheus-exporter` console
...
- **Compose ports are ephemeral, not fixed**, and each compose file has an
  ...
  `tests/integration/conftest.py` discovers the real published address via
...
`tests/integration/test_reconnect.py` actually restarts the coordinator
